...
to 256 and caps how many connections a single client address may hold open; excess connections
from that client are closed after a JSON `503 client_connection_limited` response with
`Retry-After: 1`, so one source cannot occupy every request slot (slow-loris). The request
timeout accepts 0.1 to 300
