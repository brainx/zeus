...
A ready service returns `{"schema_version":8,"status":"ready"}`. State-store
...
Schema v4 through schema v8 migrations are forward-only. V4 adds durable
...
Take a pre-v4/v5/v6/v7/v8 SQLite backup and state-tree
backup before upgrading; this is the required pre-migration database backup.
Older binaries cannot use a newer schema, including schema v8, so rolling back
